...
def normalized_camera_topics(report: dict[str, Any], camera_status: dict[str, float]) -> list[dict[str, Any]]:
    ...
    for key, topic, label in topic_specs:
        ...
        status = str(existing.get("status", "OK" if rate_hz > 0 else "FAIL")).upper()
        item = {
            "topic": topic,
            ...
            "rate_hz": rate_hz,
            "reason": translate_phrase(existing.get("reason", "" if rate_hz > 0 else "camera topic no data")),
            "suggestion": translate_phrase(existing.get("suggestion", "" if rate_hz > 0 else f"检查{label} Type-C 连接或重启 cos_teleop.service")),
        }
...
def action_timeout_seconds(action: str) -> int:
    defaults = {"check": 600, "install": 1800, "cos_setup": 1800, "service_check": 300, "service_restart": 300, DEPLOY_ACTION: 1800}
...
{restart_line}
      ...
      ansible.builtin.set_fact:
        selected_service_names: >-
          {{{{
            ...
            if service_action == 'service_restart'
            else (
    ...
    - name: Check ROS2 camera topic heartbeats
      ansible.builtin.shell: |
        ...
        hz_output="$(timeout 10 ros2 topic hz {{{{ item.topic }}}} 2>&1)"
        hz_rc=$?
        printf '%s\\n' "$hz_output"
        if printf '%s\\n' "$hz_output" | grep -Eq 'average rate:[[:space:]]*0*\.?0*([1-9][0-9]*|[0-9]*[1-9][0-9]*)'; then
          exit 0
        fi
        printf 'ros_hz_warning: no stable non-zero messages received on %s rc=%s\\n' "{{{{ item.topic }}}}" "$hz_rc"
        exit 1
      args:
        executable: /bin/bash
      loop: "{{{{ camera_topics }}}}"
    ...
    - name: Record ROS2 camera topic heartbeat status
      ...
      vars:
        ...
        camera_topic_reason: "camera topic no data"
        ...
        camera_topic_result: >-
          {{{{
            {{
              ...
              'status': ('OK' if camera_topic_ok | bool else 'FAIL'),
    ...
    - name: Initialize normalized camera topic heartbeat results
      ansible.builtin.set_fact:
        camera_topic_results_normalized: []

    - name: Normalize camera topic heartbeat results
      ansible.builtin.set_fact:
        ...
        camera_topic_result: >-
          {{{{
            ...
            | combine({{}} if existing_camera_topic.get('status', 'FAIL') == 'OK' else {{
              ...
              'detail': existing_camera_topic.get('detail', 'ros2 topic hz ' ~ item.topic ~ ' did not return data'),
    ...
    - name: Replace camera topic heartbeat results with normalized list
      ansible.builtin.set_fact:
    ...
    - name: Evaluate missing cameras
      ansible.builtin.set_fact:
        all_camera_missing: "{{ (camera_topic_results | default([]) | selectattr('status', 'equalto', 'OK') | list | length) == 0 }}"

    - name: Build service subsystem statuses
      ansible.builtin.set_fact:
        system_services_status: "{{ 'FAIL' if service_failed_items | length > 0 else 'WARN' if service_warn_items | length > 0 else 'OK' }}"
        camera_streams_status: "{{ 'FAIL' if all_camera_missing | bool else 'WARN' if camera_failed_items | length > 0 else 'OK' }}"
      ...
      ansible.builtin.set_fact:
        service_final_report:
          ...
          overall_status: >-
            {{{{
              'FAIL'
              if (service_failed_items | length > 0 or (service_action == 'service_check' and all_camera_missing | bool))
              else 'WARN'
...
def run_deploy_to_robot(ip: str, password: str, log_file: Path) -> tuple[int, str, str]:
    robot_dir = ROOT_DIR / "deploy_eva_robot检测" / "robot"
    cmd = [
        "bash",
        "-c",
        f"export REMOTE_HOST={shlex.quote(ip)} && ./deploy_to_robot.sh unitree@{shlex.quote(ip)}:/tmp/test",
    ]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def start_action(self, action: str, payload: dict[str, Any]) -> None:
        if action not in ("cos_setup", "install", "check", "service_check", "service_restart", DEPLOY_ACTION):
