...
void PicoVideoStreamer::push_mjpeg_frame(const uint8_t * data, size_t size)
{
  ...
  const cv::Mat frame = cv::imdecode(encoded_frame, cv::IMREAD_COLOR);
  if (frame.empty()) {
    TELEOP_LOG_WARN_THROTTLE(2000, "Failed to decode MJPG frame for Pico video");
    return;
  }
